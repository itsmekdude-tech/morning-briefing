...
export function ConnectPage() {
  ...
  const [mode, setMode] = useState<'gmail' | 'mock' | 'loading'>('loading');
  const pushToast = useToastStore((s) => s.push);

  useEffect(() => {
    realApi
      .getMode()
      .then((m) => setMode(m.inbox_source))
      .catch(() => setMode('mock'));
  }, []);
  ...
  const onAllow = async () => {
    ...
    try {
      if (mode === 'gmail') {
        // Real OAuth: ask backend for the consent URL, then navigate to Google
        const { authorization_url } = await realApi.getAuthStart();
        window.location.href = authorization_url;
      } else {
        // Mock mode: backend will return a stubbed connection
        const res = await realApi.connectGoogle();
        pushToast({ message: `Connected as ${res.email}` });
        navigate('/briefing');
      }
    } catch (err) {
      setLoading(false);
      pushToast({ message: `Could not start auth: ${(err as Error).message}` });
    }
  };
    ...
    <div className="min-h-screen bg-ink-900/40 flex items-center justify-center p-6">
      <div className="bg-surface-0 rounded-2xl shadow-card-dark max-w-md w-full border border-ink-300/60 p-8">
        ...
        <div className="mt-6 border border-ink-300 rounded-lg px-4 py-3 text-sm text-ink-700 flex items-center justify-between">
          <span>
            {mode === 'gmail'
              ? 'Choose your Google account on the next screen'
        ...
        <p className="mt-6 text-xs text-ink-500 leading-relaxed border-t border-ink-300/50 pt-4">
          {mode === 'gmail'
            ? 'You will be redirected to Google to confirm access. We never send mail.'
            : 'Mock mode — no real Google account is used. Set INBOX_SOURCE=gmail in backend/.env for real OAuth.'}
        </p>
        ...
        <div className="mt-6 flex justify-end gap-2">
          ...
          <Button variant="primary" onClick={onAllow} disabled={loading || mode === 'loading'}>
            {loading ? 'Connecting…' : mode === 'gmail' ? 'Continue with Google' : 'Allow'}
          </Button>
        </div>
      </div>
    </div>
